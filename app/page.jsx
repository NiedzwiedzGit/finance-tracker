...
// ═══════════════════════════════════════════════════════════════
const TAX_2026 = {
// UoP – składki pracownika (% z brutto)
ZUS_EMERY: 0.0976,     // emerytalna
ZUS_RENT: 0.015,       // rentowa
ZUS_CHOR: 0.0245,      // chorobowa
ZDROW: 0.09,           // zdrowotna (od podstawy po ZUS społ.)
// PIT
PIT_RATE_1: 0.12,
PIT_RATE_2: 0.32,
PIT_THRESHOLD: 120000, // próg II skali
PIT_FREE: 30000,       // kwota wolna
PIT_MONTHLY_REDUCTION: 300, // ulga miesięczna (PIT-2)
KUP_BASE: 250,         // koszty uzyskania przychodu
// JDG ryczałt – zwolnienie z ZUS (UoP ≥ min. krajowa)
// tylko składka zdrowotna ryczałt 2026
ZDROW_RYCZALT: {
THRESHOLD_1: 60000,    // do 60k → 498,35 zł
THRESHOLD_2: 300000,   // do 300k → 830,58 zł
AMOUNT_1: 498.35,
AMOUNT_2: 830.58,
AMOUNT_3: 1495.04,     // pow. 300k
},
RYCZALT_RATE_12: 0.12,   // ryczałt 12% dla JDG IT
};
...
// Przejście progu w tym miesiącu
...
// UoP: netto → brutto (iteracyjne przybliżenie; zbieżne w 6-8 iteracjach)
function calcUoPFromNetto(netto, yearGrossSoFar = 0) {
if (!netto || netto <= 0) return null;
// Startowe przybliżenie brutto: netto / 0.72 (typowy wskaźnik)
...
brutto -= diff * 0.9; // tłumiony krok
if (brutto <= 0) { brutto = netto; break; }
}
return calcUoP(Math.round(brutto * 100) / 100, yearGrossSoFar);
}

// JDG Ryczałt 12% – netto → przychód
function calcRyczaltFromNetto(netto, yearPrzychodSoFar = 0) {
if (!netto || netto <= 0) return null;
// zdrowotna jest stałą kwotą (nie % od przychodu), więc można liczyć iteracyjnie
let przychod = netto / 0.86; // przybliżenie: netto ≈ przychód * (1 - 0.12 - mały %)
for (let i = 0; i < 20; i++) {
...
// JDG Ryczałt 12% – zwolnienie z ZUS społ. (UoP daje pełne oskładkowanie)
// Zwraca: netto, podatek (ryczałt), składka zdrowotna
function calcRyczalt(przychod, yearPrzychodSoFar = 0) {
if (!przychod || przychod <= 0) return null;
const T = TAX_2026;

// Składka zdrowotna – wyznaczana na podstawie sumarycznego przychodu rocznego
...
// Ryczałt 12% – możliwość odliczenia 50% zapłaconej zdrowotnej od przychodu
...
// ─── Pobranie wpisów dla danego miesiąca (z uwzględnieniem stałych) ───
...
return rk <= k && !(txData[k+”*skip”] || []).includes(r.id);
})
.map(r => ({ id:“rec*”+r.id+k, type:“expense”, amount:r.amount, category:r.category, note:r.label, date:k+”-01”, isRecurring:true, recurId:r.id }));
return […manual, …recurEntries];
};

// ─── Wyliczenie podatków dla wpisu z brutto ───
// Dla UoP – potrzebny narastający YTD brutto, dla ryczałtu – YTD przychód
const calcTaxForEntry = (entry, ytdContext = {}) => {
const cat = getCat(“income”, entry.category);
if (!cat.isTaxed) return null;
if (cat.taxType === “uop”) {
// YTD przed tym miesiącem – kwoty bazy PIT
return calcUoP(entry.amount, ytdContext.uopPitBase || 0);
}
if (cat.taxType === “ryczalt12”) {
return calcRyczalt(entry.amount, ytdContext.ryczaltPrzychod || 0);
}
return null;
};

// ─── Wyliczenie YTD dla wybranego miesiąca (sumy poprzednich miesięcy w tym samym roku) ───
...
// ─── Wyliczenie pełnego rozliczenia miesiąca z podatkami ───
...
const realIncome = nettoFromTaxed + inneIncome; // realnie do kieszeni
...
// ─── Prognoza zaliczki PIT na kolejny miesiąc ───
// Na podstawie średniej z 3 ostatnich miesięcy
...
// Jeśli tryb netto – przelicz na brutto przed zapisem
let bruttoAmt = amt;
...
<style>{`@import url('https://fonts.googleapis.com/css2?family=Sora:wght@300;400;500;600;700;800&family=JetBrains+Mono:wght@400;500;600&display=swap'); *{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent;} ::-webkit-scrollbar{display:none;} input,textarea,select{outline:none;border:none;background:none;font-family:inherit;color:#eeeaf4;} button{border:none;cursor:pointer;font-family:inherit;} .card{background:rgba(255,255,255,0.038);border:1px solid rgba(255,255,255,0.07);border-radius:20px;} .row{display:flex;align-items:center;gap:12px;padding:13px 0;border-bottom:1px solid rgba(255,255,255,0.05);} .row:last-child{border-bottom:none;} .pill{border-radius:100px;padding:3px 9px;font-size:10px;font-weight:600;letter-spacing:.05em;} .green{color:#4ade80;} .red{color:#f87171;} .blue{color:#7dd3fc;} .amber{color:#fbbf24;} .violet{color:#a78bfa;} .bg-blue{background:rgba(125,211,252,.1);color:#7dd3fc;} .bg-violet{background:rgba(167,139,250,.12);color:#a78bfa;} .bg-amber{background:rgba(251,191,36,.12);color:#fbbf24;} .tab{flex:1;padding:10px 6px;background:none;font-size:11px;font-weight:600;letter-spacing:.07em;text-transform:uppercase;color:#44445a;border-radius:10px;transition:all .2s;} .tab.on{background:rgba(255,255,255,.07);color:#eeeaf4;} .nav-btn{display:flex;flex-direction:column;align-items:center;gap:3px;background:none;transition:all .2s;padding:4px 8px;} .cat-chip{border-radius:12px;padding:8px 12px;background:rgba(255,255,255,.04);border:1.5px solid transparent;cursor:pointer;transition:all .15s;display:flex;flex-direction:column;align-items:center;gap:3px;font-size:10px;color:#666680;min-width:66px;} .cat-chip.sel-exp{border-color:#f87171;background:rgba(248,113,113,.1);color:#f87171;} .cat-chip.sel-inc{border-color:#4ade80;background:rgba(74,222,128,.1);color:#4ade80;} .icon-chip{width:40px;height:40px;border-radius:12px;background:rgba(255,255,255,.05);border:1.5px solid transparent;cursor:pointer;transition:all .15s;display:flex;align-items:center;justify-content:center;font-size:20px;} .icon-chip.sel{border-color:#7dd3fc;background:rgba(125,211,252,.1);} .modal-bg{position:fixed;inset:0;background:rgba(0,0,0,.7);backdrop-filter:blur(6px);z-index:200;display:flex;align-items:flex-end;justify-content:center;} .modal{background:#111119;border-radius:28px 28px 0 0;width:100%;max-width:430px;padding:24px 22px 40px;max-height:90vh;overflow-y:auto;border-top:1px solid rgba(255,255,255,.08);} .drag-handle{width:40px;height:4px;background:rgba(255,255,255,.15);border-radius:2px;margin:0 auto 20px;} .input-box{background:rgba(255,255,255,.05);border-radius:14px;padding:14px 16px;border:1px solid rgba(255,255,255,.08);display:flex;align-items:center;gap:10px;} .input-box input{flex:1;font-size:15px;} .select-box{background:rgba(255,255,255,.05);border-radius:14px;padding:14px 16px;border:1px solid rgba(255,255,255,.08);width:100%;color:#eeeaf4;font-size:14px;-webkit-appearance:none;appearance:none;} .btn-primary{width:100%;padding:17px;border-radius:16px;font-size:15px;font-weight:700;letter-spacing:.02em;color:#fff;transition:all .15s;} .progress-bar{height:8px;border-radius:100px;background:rgba(255,255,255,.07);overflow:hidden;} .progress-fill{height:100%;border-radius:100px;transition:width .5s ease;} @keyframes up{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)}} .up{animation:up .3s ease both;} @keyframes pop{from{transform:scale(.92);opacity:0}to{transform:scale(1);opacity:1}} .pop{animation:pop .2s ease both;} .breakdown-grid{display:grid;grid-template-columns:1fr auto;gap:8px 14px;} .breakdown-grid > *:nth-child(odd){color:#888899;font-size:12px;} .breakdown-grid > *:nth-child(even){font-family:'JetBrains Mono',monospace;font-size:13px;font-weight:600;text-align:right;}`}</style>
  ...
  {tab==="dashboard" && (
    <div style={{padding:"0 0 110px",position:"relative",zIndex:1}}>
      <div style={{padding:"56px 22px 0"}}>
        <div style={{fontSize:11,color:"#44445a",letterSpacing:".1em",textTransform:"uppercase",marginBottom:6}}>Realne saldo (po podatkach)</div>
      ...
      {/* Karta podatkowa – bieżący miesiąc */}
      {(() => {
        const s = computeMonthSummary(now.getFullYear(), now.getMonth());
        const totalTax = s.pitUoP + s.ryczaltJDG;
        const totalSkladki = s.zusUoP + s.zdrowUoP + s.zdrowJDG;
        if (totalTax === 0 && totalSkladki === 0) return null;
        return (
          <div className="card" style={{margin:"14px 18px 0",padding:"18px",borderColor:"rgba(167,139,250,.15)",cursor:"pointer"}} onClick={()=>setTab("taxes")}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                <span style={{fontSize:18}}>🧾</span>
                <span style={{fontSize:14,fontWeight:600}}>Podatki i składki — {MONTHS_SHORT[now.getMonth()]}</span>
              </div>
              <span style={{fontSize:11,color:"#a78bfa"}}>→</span>
            </div>
            <div className="breakdown-grid">
              {s.bruttoUoP > 0 && (<>
                <div>UoP brutto</div><div>{fmtDec(s.bruttoUoP)}</div>
                <div style={{paddingLeft:12,color:"#666"}}>— ZUS społeczne</div><div className="violet">{fmtDec(s.zusUoP)}</div>
                <div style={{paddingLeft:12,color:"#666"}}>— Zdrowotna</div><div className="violet">{fmtDec(s.zdrowUoP)}</div>
                <div style={{paddingLeft:12,color:"#666"}}>— PIT (zaliczka)</div><div className="amber">{fmtDec(s.pitUoP)}</div>
                <div style={{paddingLeft:12,color:"#4ade80"}}>= Na rękę UoP</div><div className="green">{fmtDec(s.nettoUoP)}</div>
              </>)}
              {s.przychodJDG > 0 && (<>
                {s.bruttoUoP > 0 && <div style={{gridColumn:"1/-1",height:1,background:"rgba(255,255,255,.05)",margin:"4px 0"}}/>}
                <div>JDG przychód (12%)</div><div>{fmtDec(s.przychodJDG)}</div>
                <div style={{paddingLeft:12,color:"#666"}}>— Zdrowotna ryczałt</div><div className="violet">{fmtDec(s.zdrowJDG)}</div>
                <div style={{paddingLeft:12,color:"#666"}}>— Ryczałt 12%</div><div className="amber">{fmtDec(s.ryczaltJDG)}</div>
                <div style={{paddingLeft:12,color:"#4ade80"}}>= Na rękę JDG</div><div className="green">{fmtDec(s.nettoJDG)}</div>
              </>)}
            </div>
            <div style={{marginTop:14,padding:"12px 14px",background:"rgba(167,139,250,.08)",borderRadius:12,display:"flex",justifyContent:"space-between"}}>
              <span style={{fontSize:12,color:"#a78bfa",fontWeight:600}}>Łącznie do US/ZUS</span>
              <span className="violet" style={{fontSize:14,fontWeight:700,fontFamily:"'JetBrains Mono',monospace"}}>{fmtDec(totalTax + totalSkladki)}</span>
            </div>
          </div>
        );
      })()}
      ...
      {forecast && (forecast.total > 0) && (
        <div className="card" style={{margin:"14px 18px 0",padding:"16px 18px"}}>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
            ...
            <span style={{fontSize:13,fontWeight:600,color:"#a78bfa"}}>Prognoza – kolejny miesiąc</span>
          </div>
          <div style={{fontSize:12,color:"#888"}}>Szacowana zaliczka PIT + ryczałt</div>
          <div style={{fontSize:22,fontWeight:800,color:"#a78bfa",marginTop:4,fontFamily:"'JetBrains Mono',monospace"}}>{fmt(forecast.total)}</div>
          <div style={{fontSize:11,color:"#44445a",marginTop:4}}>na podstawie średniej z 3 ostatnich miesięcy</div>
        </div>
      )}
      ...
      <div className="card" style={{margin:"14px 18px 0",padding:"18px"}}>
        ...
        <div style={{display:"flex",gap:14,marginTop:10}}>
          <span style={{fontSize:10,color:"#4ade80",display:"flex",alignItems:"center",gap:4}}><span style={{width:8,height:8,borderRadius:2,background:"#4ade80",display:"inline-block"}}/>Na rękę</span>
          <span style={{fontSize:10,color:"#f87171",display:"flex",alignItems:"center",gap:4}}><span style={{width:8,height:8,borderRadius:2,background:"#f87171",display:"inline-block"}}/>Wydatki</span>
        </div>
      </div>
  ...
  {tab==="month" && (
    <div style={{padding:"0 0 110px",position:"relative",zIndex:1}}>
      <div style={{padding:"56px 18px 16px"}}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
          ...
          <div style={{flex:1,display:"flex",gap:8}}>
            <select className="select-box" style={{background:"rgba(255,255,255,.06)",borderRadius:12,padding:"6px 12px",fontSize:14,fontWeight:600,flex:2}}
              value={selMonth} onChange={e=>setSelMonth(Number(e.target.value))}>
              {MONTHS_FULL.map((mn,i)=><option key={i} value={i}>{mn}</option>)}
            </select>
            <select className="select-box" style={{background:"rgba(255,255,255,.06)",borderRadius:12,padding:"6px 12px",fontSize:14,fontWeight:600,flex:1}}
        ...
        {/* Pasek z brutto vs netto, jeśli były opodatkowane */}
          ...
          return (
            <div key={e.id} className="row up" style={{animationDelay:`${i*.03}s`}} onClick={()=>openEditTx(e)}>
              ...
              <div style={{textAlign:"right",flexShrink:0}}>
                <div style={{fontSize:15,fontWeight:700,color:e.type==="income"?"#4ade80":"#f87171"}}>
      ...
      <div style={{padding:"20px 18px 0",display:"flex",justifyContent:"center"}}>
        <button onClick={()=>{setEditTarget(null);setTxForm({type:"expense",amount:"",category:"food",note:"",year:selYear,month:selMonth});setModal("addTx");}}
          style={{background:"linear-gradient(135deg,#2dd4bf,#14b8a6)",color:"#fff",borderRadius:16,padding:"14px 32px",fontSize:14,fontWeight:700,boxShadow:"0 8px 24px rgba(45,212,191,.25)"}}>
          + Dodaj transakcję
        </button>
      </div>
    </div>
      ...
      <div className="card" style={{margin:"0 18px 14px",padding:"20px",borderColor:"rgba(167,139,250,.15)"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
          <div style={{background:"rgba(167,139,250,.06)",borderRadius:13,padding:"12px 12px"}}>
            ...
            <div className="amber" style={{fontSize:18,fontWeight:800,fontFamily:"'JetBrains Mono',monospace"}}>{fmt(yearTax.totalTax)}</div>
          ...
          <div style={{background:"rgba(167,139,250,.06)",borderRadius:13,padding:"12px 12px"}}>
            ...
            <div className="violet" style={{fontSize:18,fontWeight:800,fontFamily:"'JetBrains Mono',monospace"}}>{fmt(yearTax.totalSkladki)}</div>
          ...
          return (
            <div key={m} className="card" style={{padding:"14px 16px",marginBottom:8}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                ...
                <span className="violet" style={{fontSize:14,fontWeight:700,fontFamily:"'JetBrains Mono',monospace"}}>{fmt(monthTax+monthSkladki)}</span>
              </div>
  ...
  {tab==="recurring" && (
    <div style={{padding:"0 0 110px",position:"relative",zIndex:1}}>
      <div style={{padding:"56px 18px 20px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        ...
        <button onClick={()=>{setEditTarget(null);setRecurForm({label:"",amount:"",category:"bills",icon:"📄",startYear:now.getFullYear(),startMonth:now.getMonth()});setModal("addRecur");}}
          style={{width:38,height:38,borderRadius:13,background:"rgba(45,212,191,.15)",color:"#2dd4bf",fontSize:22,display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
      </div>
  ...
  {tab==="goals" && (
    <div style={{padding:"0 0 110px",position:"relative",zIndex:1}}>
      <div style={{padding:"56px 18px 20px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        ...
        <button onClick={()=>{setEditTarget(null);setGoalForm({name:"",icon:"🏠",target:"",saved:"",deadline:""});setModal("addGoal");}}
          style={{width:38,height:38,borderRadius:13,background:"rgba(251,191,36,.12)",color:"#fbbf24",fontSize:22,display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
      </div>
      ...
      <div style={{padding:"0 18px",display:"flex",flexDirection:"column",gap:14}}>
        {goals.length===0 ? (
          ...
          // Średnie odkładanie z 3 ostatnich miesięcy
          const avgSaving = (() => {
            let total = 0;
            for (let i=0;i<3;i++){
              let m = now.getMonth()-i, y=now.getFullYear();
          ...
          return (
            <div key={g.id} className="card up" style={{padding:"18px",animationDelay:`${gi*.06}s`}}>
              ...
              <div style={{display:"flex",gap:8}}>
                <button onClick={()=>{setSavingForm({goalId:String(g.id),amount:""});setModal("addSaving");}}
                  style={{flex:1,padding:"11px",borderRadius:13,background:"linear-gradient(135deg,#f59e0b,#fbbf24)",color:"#000",fontSize:13,fontWeight:700}}>+ Wpłata</button>
  ...
  {modal==="addTx" && (
    <div className="modal-bg" onClick={e=>{if(e.target===e.currentTarget){setModal(null);setEditTarget(null);}}}>
      <div className="modal">
        ...
        {/* Brutto / Netto toggle – tylko dla opodatkowanych przychodów */}
        ...
        <div className="input-box" style={{marginBottom:6}}>
          <span style={{fontSize:14,color:"#44445a",fontFamily:"'JetBrains Mono',monospace"}}>PLN</span>
          <input type="number" inputMode="decimal"
            ...
            style={{flex:1,fontSize:26,fontWeight:700,fontFamily:"'JetBrains Mono',monospace"}}/>
        ...
        {txForm.type==="income" && (txForm.category==="uop" || txForm.category==="jdg_ryczalt") && txForm.amount && (() => {
          ...
          // Reverse charge: kwota = netto, bez VAT po stronie JDG
            ...
            return (
              <div style={{marginBottom:14,padding:"12px 14px",background:"rgba(34,211,238,.06)",borderRadius:12,fontSize:12,border:"1px solid rgba(34,211,238,.15)"}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                  ...
                  <span style={{color:"#22d3ee",fontWeight:700,fontSize:11,textTransform:"uppercase",letterSpacing:".07em"}}>Reverse Charge (odwrotne obciążenie)</span>
                </div>
                <div style={{fontSize:11,color:"#888",marginBottom:10,lineHeight:1.5}}>
                  VAT rozlicza nabywca — Ty wystawiasz fakturę netto bez VAT. Kwota którą wpisujesz to Twój pełny przychód.
                </div>
                <div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:"4px 14px",fontFamily:"'JetBrains Mono',monospace"}}>
                  {isNetto && <><span style={{color:"#888"}}>Przeliczone brutto</span><span style={{color:"#eeeaf4",fontWeight:700}}>{fmtDec(result.przychod)}</span></>}
                  <span style={{color:"#888"}}>VAT (RC)</span><span style={{color:"#22d3ee",fontWeight:600}}>0 zł — nabywca</span>
                  <span style={{color:"#888"}}>Zdrowotna ryczałt</span><span className="violet">−{fmtDec(result.zdrow)}</span>
                  ...
                  <span style={{gridColumn:"1/-1",color:"#666",fontSize:10,marginTop:6,fontFamily:"'Sora',sans-serif"}}>✓ Bez ZUS społ. (UoP pokrywa) · ✓ Bez VAT (RC)</span>
                </div>
              </div>
            );
          }
          ...
          return (
            <div style={{marginBottom:14,padding:"12px 14px",background:"rgba(167,139,250,.06)",borderRadius:12,fontSize:12}}>
              <div style={{color:"#a78bfa",fontWeight:600,marginBottom:8,fontSize:11,textTransform:"uppercase",letterSpacing:".07em"}}>
                {isNetto ? "Przeliczam z netto na brutto:" : "Z brutto liczę:"}
              </div>
              {cat.taxType === "uop" && (
                <div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:"4px 14px",fontFamily:"'JetBrains Mono',monospace"}}>
                  {isNetto && <><span style={{color:"#22d3ee"}}>Brutto (obliczone)</span><span style={{color:"#22d3ee",fontWeight:700}}>{fmtDec(result.brutto)}</span></>}
                  ...
                  <span style={{color:"#4ade80",fontWeight:600}}>Na rękę</span><span className="green" style={{fontWeight:700}}>{fmtDec(result.netto)}</span>
                  {isNetto && <span style={{gridColumn:"1/-1",color:"#666",fontSize:10,marginTop:6,fontFamily:"'Sora',sans-serif"}}>✓ Zapisuję brutto {fmtDec(result.brutto)} — do celów podatkowych</span>}
                </div>
              )}
              {cat.taxType === "ryczalt12" && (
                <div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:"4px 14px",fontFamily:"'JetBrains Mono',monospace"}}>
                  {isNetto && <><span style={{color:"#22d3ee"}}>Przychód (obliczony)</span><span style={{color:"#22d3ee",fontWeight:700}}>{fmtDec(result.przychod)}</span></>}
                  <span style={{color:"#888"}}>Zdrowotna ryczałt</span><span className="violet">−{fmtDec(result.zdrow)}</span>
                  <span style={{color:"#888"}}>Ryczałt 12%</span><span className="amber">−{fmtDec(result.ryczalt)}</span>
                  <span style={{color:"#4ade80",fontWeight:600}}>Na rękę</span><span className="green" style={{fontWeight:700}}>{fmtDec(result.netto)}</span>
                  <span style={{gridColumn:"1/-1",color:"#666",fontSize:10,marginTop:6,fontFamily:"'Sora',sans-serif"}}>✓ Bez ZUS społ. (UoP pokrywa){isNetto ? ` · zapisuję przychód ${fmtDec(result.przychod)}` : ""}</span>
                </div>
              )}
            </div>
          );
        ...
        {/* Reverse Charge toggle – tylko dla JDG ryczałt */}
        {txForm.type==="income" && txForm.category==="jdg_ryczalt" && (
          <div
            onClick={()=>setTxForm(f=>({...f,reverseCharge:!f.reverseCharge}))}
            style={{
              marginBottom:14,
              padding:"13px 16px",
              borderRadius:14,
              border: txForm.reverseCharge ? "1.5px solid rgba(34,211,238,.45)" : "1.5px solid rgba(255,255,255,.08)",
              background: txForm.reverseCharge ? "rgba(34,211,238,.07)" : "rgba(255,255,255,.03)",
              cursor:"pointer",
              display:"flex",
              alignItems:"center",
              gap:12,
            ...
            <div style={{
              ...
              background: txForm.reverseCharge ? "#22d3ee" : "rgba(255,255,255,.1)",
              position:"relative", flexShrink:0, transition:"background .2s",
            }}>
              <div style={{
                position:"absolute", top:3,
                left: txForm.reverseCharge ? 21 : 3,
            ...
            <div>
              <div style={{fontSize:13,fontWeight:600,color:txForm.reverseCharge?"#22d3ee":"#888"}}>
                Reverse Charge (odwrotne obciążenie)
              </div>
              <div style={{fontSize:11,color:"#555",marginTop:2,lineHeight:1.4}}>
                {txForm.reverseCharge
                  ? "Kwota = netto · VAT rozlicza nabywca · nie odliczasz VAT"
                  : "Włącz jeśli VAT płaci Twój klient (np. usługi B2B za granicę)"}
              </div>
            </div>
          </div>
        )}
  ...
  {modal==="addRecur" && (
    <div className="modal-bg" onClick={e=>{if(e.target===e.currentTarget){setModal(null);setEditTarget(null);}}}>
      <div className="modal">
        ...
        <div className="input-box" style={{marginBottom:12}}>
          <span style={{fontSize:13,color:"#44445a",fontFamily:"'JetBrains Mono',monospace"}}>PLN</span>
          <input type="number" inputMode="decimal" placeholder="Kwota" value={recurForm.amount} onChange={e=>setRecurForm(f=>({...f,amount:e.target.value}))} style={{flex:1,fontSize:20,fontWeight:700,fontFamily:"'JetBrains Mono',monospace"}}/>
        </div>
  ...
  {(modal==="addGoal"||modal==="editGoal") && (
    <div className="modal-bg" onClick={e=>{if(e.target===e.currentTarget){setModal(null);setEditTarget(null);}}}>
      <div className="modal">
        ...
        <div className="input-box" style={{marginBottom:12}}>
          <span style={{fontSize:13,color:"#44445a",fontFamily:"'JetBrains Mono',monospace"}}>PLN</span>
          <input type="number" inputMode="decimal" placeholder="Docelowo" value={goalForm.target} onChange={e=>setGoalForm(f=>({...f,target:e.target.value}))} style={{flex:1,fontSize:20,fontWeight:700,fontFamily:"'JetBrains Mono',monospace"}}/>
        </div>

        {!editTarget && (
          <div className="input-box" style={{marginBottom:12}}>
            <span style={{fontSize:13,color:"#44445a",fontFamily:"'JetBrains Mono',monospace"}}>PLN</span>
            <input type="number" inputMode="decimal" placeholder="Już odłożono" value={goalForm.saved} onChange={e=>setGoalForm(f=>({...f,saved:e.target.value}))} style={{flex:1,fontSize:20,fontWeight:700,fontFamily:"'JetBrains Mono',monospace"}}/>
          </div>
        )}
  ...
  {modal==="addSaving" && (
    <div className="modal-bg" onClick={e=>{if(e.target===e.currentTarget)setModal(null);}}>
      <div className="modal">
        ...
        <div className="input-box" style={{marginBottom:20}}>
          <span style={{fontSize:13,color:"#44445a",fontFamily:"'JetBrains Mono',monospace"}}>PLN</span>
          <input type="number" inputMode="decimal" placeholder="Kwota" value={savingForm.amount} onChange={e=>setSavingForm(f=>({...f,amount:e.target.value}))} style={{flex:1,fontSize:26,fontWeight:700,fontFamily:"'JetBrains Mono',monospace"}}/>
        </div>
